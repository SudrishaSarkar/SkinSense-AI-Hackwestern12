...
import { SkinAnalysisResponse, SkinAnalysis, Level } from '../types';
...
function mapNewSkinAnalysisToLegacyAnalysis(newResponse: SkinAnalysisResponse): SkinAnalysis {
  ...
  const texture_notes: string[] = [...newResponse.ai_findings.texture, ...newResponse.ai_findings.other_observations];
  ...
  const triggersFromInterpretation = newResponse.combined_interpretation.match(/(dehydration|stress|hormonal phase|lack of sleep|comedogenic products|over-exfoliation|pollution|dry air)/gi);
  if (triggersFromInterpretation) {
      legacyAnalysis.probable_triggers = Array.from(new Set(triggersFromInterpretation.map(t => t.toLowerCase())));
  }
